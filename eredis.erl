...
start(#config{shell=true}=Config) ->
    register(shell, spawn_link(session, start, [shell, bucket_map(Config#config.buckets)]));
start(Config) ->
    register(server, spawn_link(server, start, [Config, bucket_map(Config#config.buckets)])).
